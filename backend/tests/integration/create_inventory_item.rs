...
mod tests {
    use crate::{
        ...
        test_helpers::{build_create_inventory_item_service, insert_owned_household},
    };
    ...
    async fn household_member_can_create_inventory_item() {
        let (service, inventory_item_repository, _category_repository, household_repository) =
        ...
        let stored = inventory_item_repository
            .find_by_id(&result, &household.id())
            .await
